...
function normalizeSite(input, index) {
  ...
  const userId = positiveInteger(input?.userId, undefined, `sites[${index}].userId`)
  const quotaPerUnit = Number(input?.quotaPerUnit ?? 500000)
  ...
  return {
    ...
    quotaPerUnit,
    balanceThreshold,
    monitoredGroups: normalizedGroups(input),
