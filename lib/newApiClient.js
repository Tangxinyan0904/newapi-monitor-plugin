...
export function createApiClient({ fetchImpl = fetch, timeoutMs = 10000 } = {}) {
  const newApi = createNewApiClient({ fetchImpl, timeoutMs })
  const sub2Api = createSub2ApiClient({ fetchImpl, timeoutMs })
  return {
    fetchSnapshot: site => (site?.type === 'sub2api' ? sub2Api.fetchSnapshot(site) : newApi.fetchSnapshot(site)),
    testConnection: site => (site?.type === 'sub2api' ? sub2Api.testConnection(site) : newApi.testConnection(site)),
    fetchDashboardSite: (site, options) => (
      site?.type === 'sub2api'
        ? sub2Api.fetchDashboardSite(site, options)
        : newApi.fetchDashboardSite(site, options)
    ),
  }
}

export const createProviderClient = createApiClient
